...
class RISCV_Disassembler( object ):
    ...
    def Disassemble( self, codeStr ):
        try:
            ...
            if common.IsLoad():
                ...
                insn = self.InstructionOp64( code )
            elif common.IsAUIPC():
    ...
    class InstructionBase( object ):
        """ RISCV distribution """
        # IMAFDZicsr_Zifencei = G extention
        RV32I = True
        #RV64I = False
        RV64I = True
        RV32M = True
    ...
    class InstructionCommon( InstructionBase ):
        ...
        def __init__( self, code ):
            ...
            self.functA = inst[-32:-28]
            self.funct5 = inst[-25:-20]
            self.funct12 = inst[-32:-20]
            self.functaq = inst[-27:-27]
            self.functrl = inst[-26:-26]
            ...
            self.J_Imm = str(hex(int(inst[-32] * 12 + inst[-20:-12] + inst[-21]
                                + inst[-31:-21] + '0', 2)))
        def IsFLoad( self ):
            return self.opCode == self.OC_FLOAD

        def IsLoad( self ):
            return self.opCode == self.OC_LOAD

        def IsFStore( self ):
            return self.opCode == self.OC_FSTORE
        ...
        def IsOp64( self ):
            return self.opCode == self.OC_OP64

        def Is64A( self ):
            return (self.opCode == self.OC_64A and self.funct3 == '011')
        
        def IsMadd( self ):
            return self.opCode == self.OC_MADD
        
        def IsMsub( self ):
            return self.opCode == self.OC_MSUB

    class Instruction64A( InstructionCommon ):
        """ RV64A ext instruction """

        def __init__( self, code ):
            RISCV_Disassembler.InstructionCommon.__init__( self, code )

        def __str__( self ):
            if self.functA == '00010' and self.RV64A:
                opType = 'lr.d '
            elif self.functA == '00011' and self.RV64A:
                opType = 'sc.d '
            elif self.functA == '00001' and self.RV64A:
                opType = 'amoswap.d '
            elif self.functA == '00000' and self.RV64A:
                opType = 'amoadd.d '
            elif self.functA == '00100' and self.RV64A:
                opType = 'amoxor.d '
            elif self.functA == '01100' and self.RV64A:
                opType = 'amoand.d '
            elif self.functA == '01000' and self.RV64A:
                opType = 'amoor.d '
            elif self.functA == '10000' and self.RV64A:
                opType = 'amomin.d '
            elif self.functA == '10100' and self.RV64A:
                opType = 'amomax.d '
            elif self.functA == '11000' and self.RV64A:
                opType = 'amominu.d '
            elif self.functA == '11100' and self.RV64A:
                opType = 'amomaxu.d '            
            else:
                opType = 'Unknown'
            asmStr = opType + 'aq' + self.functaq + 'rl' + self.functrl + self.rs1 + ', ' + self.rs2 + ', ' + self.rd
            return asmStr
    class InstructionFLoad( InstructionCommon ):
        """ RV32F Load instruction """
        ...
        def __str__( self ):
            if self.funct3 == '010' and self.RV32F:
                opType = 'flw '
            if self.funct3 == '011' and self.RV32F:
                opType = 'fld '
            if self.funct3 == '100' and self.RV32F:
                opType = 'fld '
            else:
                opType = 'Unknown'
            asmStr = opType + self.rd + ', '  + '(' + self.rs1 + ')'
            return asmStr
    ...
    class InstructionFStore( InstructionCommon ):
        ...
        def __str__( self ):
            if self.funct3 == '010' and self.RV32F:
                opType = 'fsw '
            elif self.funct3 == '011' and self.RV32F:
                opType = 'fsd '
            elif self.funct3 == '100' and self.RV32F:
                opType = 'fsq '
            else:
                opType = 'Unknown'
            asmStr = opType + self.rs2 + ', ' + self.rs1
            return asmStr
    ...
    class InstructionOp64( InstructionCommon ):
        """ Op64 RV64M instruction """

        def __init__( self, code ):
            RISCV_Disassembler.InstructionCommon.__init__( self, code )

        def __str__( self ):
            if self.funct7 == '0000001' and self.RV64M:
                if self.funct3 == '000':
                    opType = 'mulw '
                elif self.funct3 == '100':
                    opType = 'divw '
